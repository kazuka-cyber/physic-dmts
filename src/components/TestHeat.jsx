import heat from "@/utils/gerel";
import { useRouter } from "next/router";
import { useState } from "react";

export default function Testheat() {
  ...
  function handleBtn(choice) {
    if (heat[ind].correct_answer === choice) {
      setScore(score + 1);
      setCorrect([
        ...correct,
        {
          question: heat[ind].question,
          zuv: heat[ind].correct_answer,
        },
      ]);
    } else {
      setIncorrect([
  ...
  return (
    <div className="bg-[#F3F4F6] w-full lg:w-[100%]  p-6 overflow-y-auto ">
      <div className="min-h-screen  rounded-xl  sm:p-8 flex flex-col gap-10">
        ...
        {heat[ind].choices.map((choice, idx) => (
            <button
              key={idx}
